...
function generateSearchIndex(locale) {
  const items = [];
  const contentDir = path.join(DOCS_ROOT, locale);
...
function generateSearchIndexFile() {
  const ruIndex = generateSearchIndex('ru');
  const enIndex = generateSearchIndex('en');

  const outputDir = path.join(process.cwd(), 'public', 'assets', 'docs-pages', 'search');

  if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir, { recursive: true });
  }

  fs.writeFileSync(path.join(outputDir, 'ru.json'), JSON.stringify(ruIndex));

  fs.writeFileSync(path.join(outputDir, 'en.json'), JSON.stringify(enIndex));
